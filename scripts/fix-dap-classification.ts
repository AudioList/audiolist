/**
 * fix-dap-classification.ts
 *
 * Phase 3: Move stationary/non-portable products out of the DAP category.
 *
 * Usage:
 *   npx tsx scripts/fix-dap-classification.ts --dry-run
 *   npx tsx scripts/fix-dap-classification.ts
 */

import { getSupabase } from './config/retailers.ts';
import { detectDapCategory } from './scrapers/matcher.ts';
...
async function main() {
  ...
  console.log('=================================================================');
  console.log('  Phase 3: DAP Category Reclassification');
  console.log(`  Mode: ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log('=================================================================\n');

  // Load all DAP products
  ...
  while (true) {
    ...
    if (error) { console.error('Error loading DAP products:', error.message); break; }
