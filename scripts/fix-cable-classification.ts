/**
 * fix-cable-classification.ts
 *
 * Phase 1: Reclassify cable products between cable, iem_cable, and hp_cable.
 *
 * Usage:
 *   npx tsx scripts/fix-cable-classification.ts --dry-run
 *   npx tsx scripts/fix-cable-classification.ts
 */

import { getSupabase } from './config/retailers.ts';
import { detectCableSubCategory } from './scrapers/matcher.ts';
...
async function main() {
  ...
  const sourceCategories: CategoryId[] = ['cable', 'hp_cable', 'iem_cable'];

  console.log('=================================================================');
  console.log('  Phase 1: Cable Sub-Category Reclassification');
  console.log(`  Mode: ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log('=================================================================\n');

  // Load all cable products
  ...
  console.log(`Loaded ${products.length} cable products across ${sourceCategories.join(', ')}\n`);

  // Detect reclassifications
  const reclassifications: { product: ProductRow; target: CategoryId }[] = [];
  for (const p of products) {
    const detected = detectCableSubCategory(p.name, p.brand, p.category_id);
    if (detected && detected !== p.category_id) {
