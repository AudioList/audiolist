/**
 * fix-dac-amp-classification.ts
 *
 * Phase 4: Consolidate DAC/Amp products.
 * Policy: All combos (mentioning both DAC and amp) -> 'dac'.
 * Pure amps with no DAC reference stay in 'amp'.
 *
 * Usage:
 *   npx tsx scripts/fix-dac-amp-classification.ts --dry-run
 *   npx tsx scripts/fix-dac-amp-classification.ts
 */

import { getSupabase } from './config/retailers.ts';
import { detectDacAmpCategory } from './scrapers/matcher.ts';
...
async function main() {
  ...
  const sourceCategories: CategoryId[] = ['dac', 'amp'];

  console.log('=================================================================');
  console.log('  Phase 4: DAC/Amp Consolidation');
  console.log(`  Mode: ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log('=================================================================\n');

  // Load all DAC and Amp products
  ...
  console.log(`Loaded ${products.length} DAC/Amp products across ${sourceCategories.join(', ')}\n`);

  // Detect reclassifications
  const reclassifications: { product: ProductRow; target: CategoryId }[] = [];
  for (const p of products) {
    const detected = detectDacAmpCategory(p.name, p.category_id);
